...
def sentenceSplit(text):
	...
	return document.get(SentencesAnnotation)
...
def tokenize(text):
	pipeline = getPipeline()
	document = pipeline.process(text)
	tokens = document.get(TokensAnnotation)
	tokenWords = [ t.word().encode('utf8') for t in tokens ]
	return tuple(tokenWords)
...
def loadNoDuplicateWordlistFileWithTypes(wordlistPath, stopwordsFile, removeShortwords, binaryTermsFile, binaryTermsFile_out):
	wordlist = {}
	...
	# Load a word-list from a file if needed
		...
		# For the moment, duplicates are unacceptable
		#if len(duplicates) > 0:
		#	print "Duplicates:", duplicates
		#	raise RuntimeError('Found duplicate terms in term list')
		
		#print "Blanking duplicate terms (so that they will be identified but not processed in text)..."
		#for t in duplicates:
			#wordlist.delete(t)
		#	wordlist[t] = None
		#print "Completed blanking of %d duplicate terms" % len(duplicates)
