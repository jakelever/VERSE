...
trainPickle=BioNLP-ST-2016_BB-event_train.modified.pickle
devPickle=BioNLP-ST-2016_BB-event_dev.modified.pickle

echo "$parameters" > $logDir/$uniqueName.parameters

mkdir -p $outDir
$python RelationExtractor.py --trainingPickle $trainPickle --testingPickle $devPickle --outDir $outDir --parameters "$parameters" --rel_descriptions bb.description > $logDir/$uniqueName.1.log
$python Evaluate.py --goldDir ../Data/BioNLP-ST-2016_BB-event_dev.modified --testDir $outDir > $logDir/$uniqueName.results1
rm -fr $outDir

mkdir -p $outDir
$python RelationExtractor.py --trainingPickle $devPickle --testingPickle $trainPickle --outDir $outDir --parameters "$parameters" --rel_descriptions bb.description > $logDir/$uniqueName.2.log
$python Evaluate.py --goldDir ../Data/BioNLP-ST-2016_BB-event_train.modified --testDir $outDir > $logDir/$uniqueName.results2
rm -fr $outDir
